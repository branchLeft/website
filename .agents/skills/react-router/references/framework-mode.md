...
## Committed Strategy (read this first)

This app is locked to **React Router v7 Framework Mode + SSR**. These decisions are final unless explicitly revisited:

| Decision            | Value                    | Notes                                                                                                                           |
| ------------------- | ------------------------ | ------------------------------------------------------------------------------------------------------------------------------- |
| Mode                | Framework Mode           | `@react-router/dev` + `reactRouter()` Vite plugin                                                                               |
| SSR                 | Always on (`ssr: true`)  | Do not set `ssr: false`                                                                                                         |
| Route config        | Manual `app/routes.ts`   | Uses `route()`, `index()`, `layout()`, `prefix()` helpers                                                                       |
| File-system routing | Not used                 | Do not introduce `flatRoutes()`                                                                                                 |
| SPA mode            | Prohibited               | Never set `ssr: false`                                                                                                          |
| Pre-rendering       | Not yet active           | Pattern is documented in `react-router.config.ts`; opt-in per route via `prerender: ['/path']` when a route has no dynamic data |
| RSC                 | Not active (future path) | Do not introduce until `references/rsc.md` is restored                                                                          |

When adding a new route: add it to `app/routes.ts` using the helpers, create a route module under `app/routes/`, and use `loader` / `action` for server data. If the route has no dynamic data, consider adding it to `prerender` in `react-router.config.ts`.

---

Framework Mode is React Router's full-stack mode. It uses the React Router Vite plugin, route config in `app/routes.ts`, route modules, generated route types, and rendering strategies such as SSR and pre-rendering.

Use this reference after the main skill confirms this is a Framework Mode app.
...
## RSC Framework (Future Path — Not Active)

RSC (React Server Components) is the natural next step from Framework Mode and has been intentionally excluded until it stabilizes. Do not introduce `unstable_reactRouterRSC`, `@vitejs/plugin-rsc`, `"use client"` directives, or RSC route exports (`ServerComponent`, `ServerLayout`, etc.).

If RSC becomes appropriate, restore `references/rsc.md` and read it alongside this file before making any changes.
